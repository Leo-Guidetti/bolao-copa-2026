...
// overall estimado -> preço (faixa absoluta de nível)
const baseOverall=(team)=>Math.round(82 - rankOf(team)*(14/47));   // 82 (melhor) .. 68 (pior)
const starOverall=(sv)=> sv>=8?90: sv>=5?86: sv>=3?81: 0;
const priceFromOverall=(o)=> o>=88?8: o>=84?5: o>=80?3: o>=76?2: 1;

const all=await prisma.player.findMany({select:{id:true,name:true,team:true,position:true,price:true}});
const np=new Map();
for(const pl of all){
  const sv=starVal(pl.team,pl.name);
  const ov=Math.max(baseOverall(pl.team), starOverall(sv));
  np.set(pl.id, priceFromOverall(ov));
}
// piso: melhor jogador de cada seleção >= 3
const best={};
for(const pl of all){const sc=starVal(pl.team,pl.name)*100+(ORDER.length-rankOf(pl.team));if(!best[pl.team]||sc>best[pl.team].sc)best[pl.team]={id:pl.id,sc};}
for(const t in best){if(np.get(best[t].id)<3)np.set(best[t].id,3);}
...
console.log("=== por posição ===");
for(const pos of POS)console.log(pos.padEnd(4),fmt(hist(pl=>pl.position===pos)));
console.log("=== total ===\n",fmt(hist(()=>true)));
const teams={};for(const pl of all){(teams[pl.team]??={tot:0,t3:0,t5:0,t8:0});const v=np.get(pl.id);teams[pl.team].tot+=v;if(v>=3)teams[pl.team].t3++;if(v>=5)teams[pl.team].t5++;if(v>=8)teams[pl.team].t8++;}
const ta=Object.entries(teams).sort((a,b)=>b[1].tot-a[1].tot);
console.log("Seleções sem ≥5¢:",ta.filter(x=>x[1].t5===0).length);
console.log("Top 8 (tot|≥3|≥5|8¢):");
for(const [t,v] of ta.slice(0,8))console.log("  "+t.padEnd(12),`tot:${String(v.tot).padStart(3)} ≥3:${v.t3} ≥5:${v.t5} 8:${v.t8}`);
console.log("Exemplos fracos:");
for(const [t,v] of [ta[20],ta[34],ta[47]])console.log("  "+t.padEnd(12),`tot:${v.tot} ≥3:${v.t3} ≥5:${v.t5}`);
let chg=0;for(const pl of all)if(np.get(pl.id)!==pl.price)chg++;console.log("Mudanças:",chg);
if(APPLY){const ups=[...np.entries()].map(([id,price])=>({id,price}));for(let i=0;i<ups.length;i+=50)await prisma.$transaction(ups.slice(i,i+50).map(u=>prisma.player.update({where:{id:u.id},data:{price:u.price}})));console.log("✅ aplicado");}

  ...
  for (const ev of target) {
    ...
    for (const { t, cats } of stats) {
      ...
      const data = {
        ...
        shootOutSaved: firstGame ? 0 : cval(cats, "goalKeeping", "shootOutKicksSaved"),
        blockedShots: firstGame ? 0 : cval(cats, "defensive", "outfielderBlock"), // bloqueio FEITO pelo jogador (não o chute dele bloqueado); só da 2ª rodada
        foulsSuffered: firstGame ? 0 : cval(cats, "general", "foulsSuffered"), foulsCommitted: firstGame ? 0 : cval(cats, "general", "foulsCommitted"),

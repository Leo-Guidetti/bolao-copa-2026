...
export async function computeStandings() {
  ...
  const rows = participants.map((p) => {
    ...
    for (const bet of p.bets) {
      ...
      if (m.finished && m.homeScore != null && m.awayScore != null && bet.homeGuess === m.homeScore && bet.awayGuess === m.awayScore) cravadas++;
    }
    ...
    return { participantId: p.id, name: p.name, paid: !!p.paid, betPts, squadPts, cravadas, squadCost, squadOver, hasSquad: !!p.squad };
  });
